...
#This processes all non amplification data.
...
def analysis(job, sliceno, slices):
    ...
    for epo in options.epoch:
        for sub in sublist:
            ...
            fname = job.input_filename(f'./{sub}/6-ClassificationData/{sub}_{epo}_data.mat')
            cdata = loadmat(fname, chars_as_strings = True, simplify_cells = True)[f'{sub}_{epo}_data']
            ...
            #job.save({f'{sub}_{epo}_datapart': datapart}, directory + f'{sub}_{epo}_datapart.pickle')
            ...
            job.save({f'{sub}_{epo}_crossvalclass': crossval}, directory + f'{sub}_{epo}_crossvalclass.pickle')
            ...
            job.save({f'{sub}_{epo}_crossvalclass_performance': crossval_perf}, directory + f'{sub}_{epo}_crossvalclass_performance.pickle')
